...
const ADMIN_NAV: NavItem[] = [
  ...
  { id: "admin_portal", label: "Data Entry", icon: <FolderOpen size={16} />, href: "/admin/data-entry" },
];
...
export default function PortalLayout({ children, isAdmin = false }: PortalLayoutProps) {
  ...
  const { effectiveTier, impersonatingTenantId, setImpersonatingTenantId, setEffectiveTier } = usePortal();
  const { data: tenant } = trpc.tenant.me.useQuery(undefined, { enabled: !isAdmin && !impersonatingTenantId });
    ...
    <div className="flex h-screen bg-background overflow-hidden">
      ...
      <aside className="flex flex-col w-44 shrink-0 border-r border-sidebar-border" style={{ backgroundColor: "oklch(0.09 0.005 240)" }}>
        ...
        <div className="flex items-center gap-2 px-4 py-4 border-b border-sidebar-border">
          ...
          {!isAdmin && !impersonatingTenantId && tenant?.companyName && (
