                ...
                <div className="divide-y divide-border">
                  {(logs ?? []).map((log, i) => (
                    <div key={log.id} className="flex items-center gap-3 px-5 py-3">
                      ...
                      <span className="text-sm font-semibold text-primary">{parseFloat(log.hours).toFixed(1)}h</span>
                      <span className="text-xs text-muted-foreground w-12 text-right">
                        {totalHours > 0 ? `${((parseFloat(log.hours) / totalHours) * 100).toFixed(0)}%` : "—"}
                      </span>
                    </div>
                  ))}
                </div>
                ...
                <div className="divide-y divide-border">
                  {delegationItems.map((log) => (
                    <div key={log.id} className="px-5 py-4">
                      ...
                      <p className="text-xs text-amber-300/80 mt-1">{log.delegationSuggestion}</p>
                    </div>
                  ))}
                </div>
              </CardContent>
