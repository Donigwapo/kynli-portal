...
export default function SalesTracker() {
  ...
  const pct = sales && (sales.goal_clients ?? 0) > 0
    ? Math.min(100, Math.round(((sales.signed_clients ?? 0) / (sales.goal_clients ?? 1)) * 100))
    : 0;

  const pieData = sales
    ? [
        { name: "Referral", value: sales.referral_count ?? 0 },
        { name: "Outbound", value: sales.outbound_count ?? 0 },
      ]
    : [];
        ...
        <>
          ...
          <Card className="bg-card border-border">
            ...
            <CardContent className="pt-5">
              <div className="flex items-end justify-between mb-3">
                <div>
                  <p className="text-5xl font-bold text-foreground">{sales.signed_clients ?? 0}</p>
                  <p className="text-sm text-muted-foreground mt-1">clients signed</p>
                </div>
                <div className="text-right">
                  <p className="text-2xl font-bold text-muted-foreground">{sales.goal_clients ?? 0}</p>
              ...
              <p className="text-xs text-muted-foreground mt-2">
                {(sales.goal_clients ?? 0) - (sales.signed_clients ?? 0) > 0
                  ? `${(sales.goal_clients ?? 0) - (sales.signed_clients ?? 0)} more to reach goal`
                  : "Goal reached! 🎉"}
              </p>
            </CardContent>
          </Card>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Card className="bg-card border-border">
              ...
              <CardContent>
                {(sales.referral_count ?? 0) === 0 && (sales.outbound_count ?? 0) === 0 ? (
            ...
            <Card className="bg-card border-border">
              ...
              <CardContent className="space-y-4 pt-2">
                <div className="flex items-center justify-between p-3 rounded-lg bg-primary/5 border border-primary/20">
                  ...
                  <div className="text-right">
                    <p className="text-xl font-bold text-primary">{sales.referral_count ?? 0}</p>
                    <p className="text-xs text-muted-foreground">
                      {(sales.signed_clients ?? 0) > 0 ? `${Math.round(((sales.referral_count ?? 0) / (sales.signed_clients ?? 1)) * 100)}%` : "—"}
                    </p>
                  </div>
                </div>
                <div className="flex items-center justify-between p-3 rounded-lg bg-violet-500/5 border border-violet-500/20">
                  ...
                  <div className="text-right">
                    <p className="text-xl font-bold text-violet-400">{sales.outbound_count ?? 0}</p>
                    <p className="text-xs text-muted-foreground">
                      {(sales.signed_clients ?? 0) > 0 ? `${Math.round(((sales.outbound_count ?? 0) / (sales.signed_clients ?? 1)) * 100)}%` : "—"}
                    </p>
                  </div>
                </div>
                <div className="flex items-center justify-between p-3 rounded-lg bg-muted/30 border border-border">
                  ...
                  <p className="text-xl font-bold text-foreground">{sales.signed_clients ?? 0}</p>
                </div>
              </CardContent>
            </Card>
          </div>
